...
import { findNodeHandle, View, PanResponder, Alert } from "react-native";
...
const copilot = ({
  ...
  class Copilot extends Component<any, State> {
    ...
    enableTouchesInTarget = ({ nativeEvent }) => {
      ...
      const insideWidth =
        this.size.x < touchX && touchX < this.size.x + this.size.width;
      const insideHeight =
        this.size.y < touchY && touchY < this.size.y + this.size.height;
      return !insideWidth || !insideHeight;
    };

    panResponder = PanResponder.create({
      onMoveShouldSetPanResponderCapture: this.enableTouchesInTarget,
      onStartShouldSetPanResponderCapture: this.enableTouchesInTarget,
    ...
    progress = () =>
      setTimeout(this.isLastStep() ? this.stop : this.next, progressDelay);

    async moveToCurrentStep(): void {
      this.size = await this.state.currentStep.target.measure();
      await this.modal.animateMove({
        width: this.size.width + OFFSET_WIDTH,
        height: this.size.height + OFFSET_WIDTH,
        left: this.size.x - OFFSET_WIDTH / 2,
        top: this.size.y - OFFSET_WIDTH / 2 + verticalOffset
      });
    }
